...
# ╔═╡ c58ad299-f69f-4b4f-b0e5-85796873cc83
md"""*Paste in your bearer key here*: $(@bind bkey TextField(placeholder="sf-XXXXXXXXX"))"""

# ╔═╡ c88402c7-0107-4eae-aadd-641ecfd87bc3
md"""# Reading Augustine with help from ChatGPT"""
...
NB: assumes that the globals  `form` and `lemma` ave been defined.
"""
function lemmatization()
		string("**Dictionary form**: *", form, "* is from *", lemma, "*")
end

# ╔═╡ bda730a0-c842-4936-8bbc-453ffb702b3d
isempty(bkey) || isempty(word) ? nothing : lemmatization() |> Markdown.parse

# ╔═╡ 15a4ae72-9d04-4da5-8d8d-fd26d67fd825
lexiconprompt = """
I'm trying to interpret an article in Lewis-Short's Latin dictionary.  Can you extract 3 pieces of information for me? First, the part of speech. Second, the morphological information necessary for each part of speech: for verbs, principal parts and conjugation; for nouns, nominative singular, genitive singular and gender; for adjectives, nominative singular in masculine, feminine and neuter; for prepositions, the cases it can be constructed with; for other parts of speech, the label "uninflected".  Third, a brief English definition (just a few words or phrases). 

Please format the reply in a SINGLE line of delimited text, using the pipe character for the column delimiter: put the part of speech in the first column then morphological information in the second column and the summary definition in the third column. Here is the article:
"""
...
manifest_format = "2.0"
project_hash = "9444e7397cc18ad11d9e2c8423882b7fdcad5214"
